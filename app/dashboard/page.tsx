...
const columns = [
  ...
  { title: "Clock in note", dataIndex: "clockInNote", key: "clockInNote", ellipses: true},
  { title: "Clock out", dataIndex: "clockOut", key: "clockOut" },
  { title: "Duration", dataIndex: "duration", key: "duration" },
  { title: "Clock out note", dataIndex: "clockOutNote", key: "clockOutNote", ellipsis: true },
];
...
export default function CareWorkerDashboard(){
  ...
  const [clockedInNote, setClockedInNote] = useState("");
  const [clockedOutNote, setClockedOutNote] = useState("");
  ...
  const formatShifts = (rawShifts: ShiftType[]): ShiftRow[] => {
    ...
    return rawShifts.map((shift, index) => {
      ...
      return {
        key: shift.id ?? index,
        date: clockInDate.toLocaleTimeString(),
  ...
  const loadUserAndShifts = async () => {
    ...
    // to check if user is currently on shift or not
      ...
      const response = await fetch("/api/graphql", {
        ...
        body: JSON.stringify({
          ...
          variables: {
            ...
            clockInNote: clockedInNote || null,
          },
        }),
      });
      ...
      const response = await fetch("/api/graphql", {
        ...
        body: JSON.stringify({
          ...
          variables: {
            ...
            clockOutNote: clockedOutNote || null,
          },
        }),
      });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-brand-bg flex items-center justify-center">
        <p className="font-inter text-brand-muted">
          Loading...
        </p>
      </div>
    );
      ...
      <main className="max-w-5xl mx-auto px-6 pb-16">
        ...
        <div className="bg-white rounded-2xl border border-brand-border p-8 mb-8">
          ...
          <textarea
            ...
            value={isClockedIn ? clockedOutNote : clockedInNote}
            onChange={(e) => {isClockedIn ? setClockedOutNote(e.target.value) : setClockedInNote(e.target.value)}}
